...
The reason I am using the SentimentAnalysis package is that it operates differently from the tidytext library - it provides summary statistics for weighted scores using a complex algorithm which considers not only a term within a discourse but it's position.  This may help us more reliably determine whether an observation is predominantly positive or negative.
...
Not surprisingly, the finance dictionaries have medians and modes of 0 - there are few matches between the dictionaries and words in the play synopses.  The QDAP does better, and the QDAP analysis and the Harvard analysis are relatively similar.  They both find the plays, on average, to be mildly positive.

#### We compare to the Harvard dictionary to the dictionaries we used from Tidytext.
...
While the data remains suggestive that more positive shows do better, regression analysis fails to establish a correlation at the 95% level.
...
x
```
The Bing dataset comes to the astonishing conclusion that 60% of Broadway plays are negative in sentiment!
...
A satirical farce about an eccentric millionaire who falls intensely in love with a mysterious stranger has 7 negative words and only one positive one.  Even "funny" is considered negative here.
...
Sentiment analysis is an interesting tool but has its limitations.  As the Bing vs Harvard analysis shows, different dictionaries and algorithms can produce different results.  As custom context specific dictionaries emerge, analysis will surely improve.
