...
MedicaidTotal <- dfMedicaidEnrollment %>%
  filter(economic_region == "New York City" & eligibility_year=="2014") %>%
  group_by(economic_region) %>%
  summarize(Total = sum(as.numeric(recipients)))

Stats_DeathRate <-dfMortality2011 %>%
  filter(county_name=="Richmond" | county_name=="New York State")

Stats_Indicators <- dfDiabetesIndicators %>%
  filter(yearstart=="2018" & locationabbr=="NY")
...
write.csv(dfErVisitsByZip, "D:\\Buffer\\1.csv") 
...
dfPopAndPovertyLevel %<>%
  filter(str_length(V2)==5 & V4 == "Staten Island, New York")

write.csv(dfPopAndPovertyLevel, "D:\\Buffer\\2.csv") 
...
dfA1CAndDemographics <- read.csv("D:\\RStudio\\CUNY_607\\Projects\\Final Project\\InitialA1CAndDemographics.csv", encoding = "UTF-8")


dfA1CAndDemographics$Initial.A1C <- as.numeric(dfA1CAndDemographics$Initial.A1C)
...
dfA1CAndDemographics %<>% 
  mutate(BlackOrNot = ifelse(Race.and.Ethnicity=="Black/African American", 1, 0)) %<>%
  mutate(MaleOrNot = ifelse(Gender=="Male", 1, 0)) %>%
  mutate(A1CAboveMean = ifelse(Initial.A1C > 6, 1, 0))
...
rBase1 <- lm(Initial.A1C ~ BlackOrNot + MaleOrNot + Age,  data = dfA1CAndDemographics)
summary(rBase1)
