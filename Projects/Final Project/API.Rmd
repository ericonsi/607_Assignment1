...
We begin with the MedicaidByZip file, a NYDOH file with over a million records.  We filter it down to the records we want (the most recent diabetes records from Staten Island) and load it into the database. We also load a second  NY DOH file, dfAdultsDiagnosed which has information about Diabetes diagnoses throughout New York State.
...
dfZipInfo10 <- dfZipInfo_raw %>%
  ...
  na.omit() %>%
  group_by(zip_code) %>%
  summarize(n=n(), ave_A1C=mean(InitialA1C), pov=mean(percent_below_poverty), pop=mean(population), NoInternet=mean(NoInternetAccessPercentageOfHouseholds), percent_ip = sum(ip_admits/population), percent_er=sum(er_visits/population), )
...
kable(head(dfZipInfo10), "html") %>% kable_styling("striped") %>% scroll_box(width = "100%")
```
...
kable(head(dfNeedsInfo), "html") %>% kable_styling("striped") %>% scroll_box(width = "100%")
...
```{r}
kableExtra::kable(head(dfNeedsInfo2), "html") %>% kable_styling("striped") %>% scroll_box(width = "100%")
...
kableExtra::kable(head(dfA1CAndDemographics), "html") %>% kable_styling("striped") %>% scroll_box(width = "100%")

```


## Analysis - Poverty and Diabetes
...
The link between diabetes and poverty can be difficult to get at directly, as many poorer people who have diabetes are not aware of it, and diabetes incidence is often not available in such a way it can be correlated poverty.  However, if we examine Diabetes ER and hospital admissions against poverty the correlation is unmistakable - however, there are many confounding variables here, including the fact that poor communites tend to use the hospital for primary care to a far greater degree than more affluent communities:
...
a <- ggplot(dfZipInfo_SI, aes(pov, percent_ip)) +
  ...
  ggtitle("Poverty by % Admitted to the Hospital With Diabetes") +
  my_theme()




b <- ggplot(dfZipInfo_SI, aes(pov, percent_er)) +
  geom_point() +
    stat_smooth(method = "lm", se = FALSE) +
  ggtitle("Poverty by % Going to the ER With Diabetes") +
  my_theme()
...
a <- ggplot(dfZipInfo_SI, aes(pov, percent_ip)) +
  ...
  ggtitle("Poverty by % Admitted to the Hospital With Diabetes") +
  my_theme() +
  geom_hline(yintercept=21.5, size=2, color='red', linetype='dotted')
...
```
Looking at indicators of poverty such as lack of internet shows the same pattern.

```{r}



ggplot(dfZipInfo_SI, aes(NoInternet, percent_er)) +
  geom_point() +
    stat_smooth(method = "lm", se = FALSE) +
  ggtitle("No Internet by Percent of Medicaid Recipients Going to the ER") +
  my_theme()
...
The previous graph suggests that poverty affects individual in a direct way - lack of internet may lead to lack of knowlege and poor health choices.

Our next step is to test this hypothesis with our survey data. We will look to see if incidence of diabetes and particular economic and social needs are correlated.
...
Those clients who cannot afford basic needs have a higher incidence of Diabetes in both the CHi2 and regression analysis.  However, we cannot reject the null hypothesis for other needs.

We can also see if we find the same pattern for hypertension clients.

```{r}

dfNeedsInfo_Any <- na.omit(dfNeedsInfo)
...
Prediciton for A1C and demographics
...
data.rose <- ROSE::ROSE(A1CAboveMean ~ BlackOrNot + MaleOrNot + Age, data=train, seed=1)$data
model <- glm(A1CAboveMean ~ BlackOrNot + MaleOrNot + Age, family = "binomial", data=data.rose)

#fit logistic regression model

model1 <- glm(A1CAboveMean ~ BlackOrNot + Age, family = "binomial", data=train)
#disable scientific notation for model summary
options(scipen=999)
...
Predictions <- cbind(p2, p3)
Predictions$predicted <- ifelse(Predictions$predicted >.864, 1, 0)
Predictions$predicted=as.factor(Predictions$predicted)
