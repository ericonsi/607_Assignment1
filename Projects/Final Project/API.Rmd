...
We will recode the questions so that 1 means 'has a need' and 0 means 'does not have a need' fro all variables.
...
We begin with the MedicaidByZip file, a NYDOH file with over a million records.  We filter it down to the records we want (the most recent diabetes records from Staten Island) and load it into the database. We also load a second  NY DOH file, dfAdultsDiagnosed.
...
## Create Three Dataframes from the SQL Server Database
...
2. dfAllChronicAndNeeds, a dataframe with 474 observations at the client level with information on needs from the survey, chronic diseases, demographics and initial A1C. 
...
dfAllChronicAndNeeds <- dfAllChronicAndNeeds_raw %>%
    group_by(ClientID) %>%
...
However, when we screen clients in our programs at my workplace we find the incidence of Diabetes is a much higher 14%.  What is happening here?

```{r}

dfDiab <- dfAllChronicAndNeeds %>%
...
ggplot(dfZipInfo_SI, aes(pov, percent_ip)) +
  geom_point() +
      stat_smooth(method = "loess", se = FALSE) +
  ggtitle("Poverty by Percent of Medicaid Recipients Admitted to the Hospital")

rBase1 <- lm( percent_ip ~ pov,  data = dfZipInfo_SI)
summary(rBase1)


ggplot(dfZipInfo_SI, aes(pov, percent_er)) +
  geom_point() +
    stat_smooth(method = "loess", se = FALSE) +
  ggtitle("Poverty by Percent of Medicaid Recipients Going to the ER")

rBase1 <- lm(percent_er ~ pov,  data = dfZipInfo_SI)
summary(rBase1)


ggplot(dfZipInfo_SI, aes(pov, ave_A1C)) +
  geom_point() +
  stat_smooth(method = "loess", se = FALSE) +
  ggtitle("Poverty by Average A1C")

rBase1 <- lm(ave_A1C ~ pov,  data = dfZipInfo_SI)
summary(rBase1)
...
rBase1 <- lm(percent_er ~ NoInternet,  data = dfZipInfo_SI)
summary(rBase1)
...
data.rose <- ROSE::ROSE(HypertensionOrDiabetesOrNot ~ Need_AffordNeeds + Need_HighSchool, data=train, seed=1)$data


model <- glm(HypertensionOrDiabetesOrNot ~ Need_AffordNeeds + Need_HighSchool, family="binomial", data=data.rose)

table(data.rose$HypertensionOrDiabetesOrNot)


#fit logistic regression model
model1 <- glm(HypertensionOrDiabetesOrNot ~ Need_Job + Need_AffordNeeds + Need_HighSchool, family="binomial", data=train)

# generating data according to ROSE: p=0.5 as default

model2 <- glm(HypertensionOrDiabetesOrNot ~ Need_HighSchool + Need_AffordNeeds, family="binomial", data=train)
...
model3 <- glm(DiabetesOrNot~ Need_Sad + Need_HelpUnderstanding + Need_Childcare + Need_Clothes + Need_LoseHousing + Need_SafePlace + Need_Job + Need_AffordNeeds + Need_HighSchool + Need_Transport + Need_Safe, family = "binomial", data=train)
...
Predictions$predicted <- ifelse(Predictions$predicted >.64, 1, 0)
